...
function buildGui()
    ...
    local BG = Color3.fromRGB(18,3,3)
    local BG2 = Color3.fromRGB(30,5,5)
    local ROW_BG = Color3.fromRGB(24,4,4)
    local ROW_BORDER = Color3.fromRGB(110,20,20)
    local GRAY = Color3.fromRGB(100,35,35)
    local INP = Color3.fromRGB(26,5,5)
    local OFF = Color3.fromRGB(42,7,7)
    local TAB_ACTIVE = Color3.fromRGB(255,55,55)
    local TAB_INACT = Color3.fromRGB(170,35,35)
    local SECT_LBL = Color3.fromRGB(255,120,120)
    local HOV = Color3.fromRGB(48,8,8)
    local ON_COLOR = Color3.fromRGB(255,55,55)
    local STROKE_COLOR = Color3.fromRGB(105,18,18)
    ...
    profileImage.Image = (getcustomasset and pcall(function() return getcustomasset("prime_profile.jpeg") end)) and getcustomasset("prime_profile.jpeg") or "rbxassetid://0"
    ...
    local creatorLabel = Instance.new("TextLabel", topBar)
    creatorLabel.Size = UDim2.new(1, -80 - 60, 0, 16)
    creatorLabel.Position = UDim2.new(0, 64, 0, 31)
    creatorLabel.BackgroundTransparency = 1
    creatorLabel.Text = "by speedsisno"
    creatorLabel.TextColor3 = Color3.fromRGB(190, 190, 190)
    creatorLabel.Font = Enum.Font.Gotham
    creatorLabel.TextSize = 11
    creatorLabel.TextXAlignment = Enum.TextXAlignment.Left
    creatorLabel.ZIndex = 30

    local grad = Instance.new("UIGradient", titleLabel)
    grad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(255,180,180)),
        ColorSequenceKeypoint.new(0.45, Color3.fromRGB(255,55,55)),
        ColorSequenceKeypoint.new(0.55, Color3.fromRGB(205,35,35)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(135,15,15))
    })
    grad.Rotation = 0
    ...
    closeBtn.BackgroundColor3 = Color3.fromRGB(34,5,5)
    ...
    closeBtn.MouseLeave:Connect(function()
        TS:Create(closeBtn, TweenInfo.new(0.12), {TextColor3 = WHITE, BackgroundColor3 = Color3.fromRGB(34,5,5)}):Play()
    end)
    ...
    tabContent.Size = UDim2.new(1, -16, 1, -54 - 42)
    tabContent.Position = UDim2.new(0, 8, 0, 54 + 34)
    ...
    local function animPill(pill, dot, on)
        TS:Create(pill, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {BackgroundColor3 = on and WHITE or OFF}):Play()
        ...
        if stroke then
            TS:Create(stroke, TweenInfo.new(0.2), {Color = on and WHITE or ROW_BORDER, Transparency = on and 0 or 0.6}):Play()
        end
    end
    ...
    do
        ...
        delBtn.BackgroundColor3 = Color3.fromRGB(80, 0, 0)   
        ...
        delBtn.MouseButton1Click:Connect(function()
            ...
            if deleteState == 0 then
                deleteState = 1
                delBtn.Text = "CONFIRM?"
                delBtn.BackgroundColor3 = Color3.fromRGB(150, 20, 20) 
...
function createMobilePanel()
    ...
    local WHITE = Color3.fromRGB(255,255,255)
    local INACTIVE_BG = Color3.fromRGB(24,4,4)
    local INACTIVE_TEXT = Color3.fromRGB(225,205,240)
    local STROKE_COLOR = Color3.fromRGB(105,18,18)
    local ACTIVE_BG = Color3.fromRGB(255,55,55)
    local ACTIVE_TEXT = Color3.fromRGB(20,5,28)
    local buttons = {}
...
-- PRIME UB STARTUP SOUND
-- Requires an executor that supports getcustomasset/getsynasset.
local function playPrimeStartupSound()
    local assetFn = getcustomasset or getsynasset
    if not assetFn then return end
    local ok, soundAsset = pcall(function() return assetFn("prime_startup.mp3") end)
    if not ok or not soundAsset then return end
    local sound = Instance.new("Sound")
    sound.Name = "PrimeUBStartup"
...
if _G.PrimeUBSpeedIndicator then return end
_G.PrimeUBSpeedIndicator = true
...
local function setupSpeedIndicatorHead(char)
    ...
    local oldBB = head:FindFirstChild("PrimeUBSpeedIndicator")
    if oldBB then oldBB:Destroy() end

    local bb = Instance.new("BillboardGui", head)
    bb.Name = "PrimeUBSpeedIndicator"
